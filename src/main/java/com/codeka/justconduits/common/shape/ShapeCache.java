...
public class ShapeCache {
  ...
  private final Cache<ShapeCacheKey, SelectionShape> selectionShapeCache =
      CacheBuilder.newBuilder().maximumSize(64).expireAfterAccess(60, TimeUnit.SECONDS).build();
  ...
  @Nonnull
  public SelectionShape getSelectionShape(ConduitBlockEntity conduitBlockEntity, Callable<SelectionShape> loader) {
    try {
      return selectionShapeCache.get(new ShapeCacheKey(conduitBlockEntity), loader);
    } catch (ExecutionException e) {
      // Should not happen.
      throw new RuntimeException(e);
    }
